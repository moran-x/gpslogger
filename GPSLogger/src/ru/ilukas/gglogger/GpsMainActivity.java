...
import ru.ilukas.gglogger.GpsSkyActivity;
...
import ru.ilukas.gglogger.loggers.FileLoggerFactory;
import ru.ilukas.gglogger.loggers.IFileLogger;
import ru.ilukas.gglogger.senders.FileSenderFactory;
import ru.ilukas.gglogger.senders.IFileSender;
import ru.ilukas.gglogger.senders.dropbox.DropBoxAuthorizationActivity;
import ru.ilukas.gglogger.senders.dropbox.DropBoxHelper;
import ru.ilukas.gglogger.senders.email.AutoEmailActivity;
import ru.ilukas.gglogger.senders.ftp.AutoFtpActivity;
import ru.ilukas.gglogger.senders.gdocs.GDocsHelper;
import ru.ilukas.gglogger.senders.gdocs.GDocsSettingsActivity;
import ru.ilukas.gglogger.senders.osm.OSMHelper;
import ru.ilukas.gglogger.senders.opengts.OpenGTSActivity;

import java.io.File;
import java.util.*;

public class GpsMainActivity extends SherlockActivity implements
	...
	private void ShowWhatsNew() {
		...
		if (currentVersionNumber > savedVersionNumber) {
			if (!Utilities.IsNullOrEmpty(gdocsKey)) {
				Utilities
						.MsgBox("Google Docs users, please note",
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case R.id.mnuEmail:
			SelectAndEmailFile();
			break;
		case R.id.mnuSatInfo:
			Intent GpsStatus = new Intent(getApplicationContext(),
					GpsStatusActivity.class);
			startActivity(GpsStatus);
			break;
		case R.id.mnuMap:
			Intent GpsMapActivity = new Intent(getApplicationContext(), 
					GpsMapActivity.class);
			startActivity(GpsMapActivity);
			break;
		case R.id.mnuSatRadar:
			Intent GpsSatRadar = new Intent(getApplicationContext(), 
					GpsSkyActivity.class);
			startActivity(GpsSatRadar);
		...
		case R.id.mnuEmailnow:
			...
			break;
		case R.id.mnuFAQ:
			Intent faqtivity = new Intent(getApplicationContext(),
					Faqtivity.class);
	...
	private void UploadToDropBox() {
		...
		if (!dropBoxHelper.IsLinked()) {
			startActivity(new Intent("ru.ilukas.gglogger.DROPBOX_SETUP"));
			return;
		}
